...
//   - DTLS update flow:
//       Initiator: Req -> Resp -> Fin -> ACK (rx bumped on Resp; tx bumped on ACK)
//       Responder: on Fin: bump rx+tx and send ACK
//
...
//   - bounded retransmissions for Req and Fin
...
#endif
/* Bounded wait while acting as responder (waiting for peer Fin). */
...
proctype PeerA() priority 1
{
    ...
    byte resp_retries = RESP_RETRIES;
...
WAIT_RESP:
    ...
    :: (!PHA_PENDING && deferred_resp && resp_retries > 0) ->
        A2B_SEND(Resp, tx, local_kx, true);
        resp_sent = true;
        resp_retries--
    ...
    :: A_RECV(KeyUpdate, e, kx, acc) -> unexpected = true; break
    :: A_RECV(UnknownEKU, e, kx, acc) -> unexpected = true; break
    :: A_RECV(EarlyEKU, e, kx, acc) -> unexpected = true; break
    :: A_RECV(BadGroupReq, e, kx, acc) -> illegal_parameter = true; break
    :: A_RECV(BadGroupResp, e, kx, acc) -> illegal_parameter = true; break
    ...
    :: (!PHA_PENDING && updating && initiated && !req_acked && req_retries > 0 && len(to_a) == 0) ->
        A2B_SEND(Req, tx, local_kx, true);
        req_retries--
    :: (!PHA_PENDING && updating && initiated && !req_acked && req_retries == 0 && len(to_a) == 0) ->
        ...
        updating = false;
        goto ABORT_A
    /* waiting as responder for peer Fin: bounded timeout only if no inbound
    ...
    :: A_RECV(Req, e, kx, acc) ->
        ...
        :: ((e == rx) || (retain_old && e == old_rx)) ->
            ...
            :: else ->
                ...
                :: (kx == peer_kx_active) ->
                    if
                    :: (deferred_resp && !resp_sent) -> A2B_SEND(ACK, tx, 0, true)
                    :: else -> A2B_SEND(Resp, tx, local_kx, true)
                    fi
    ...
    :: A_RECV(Resp, e, kx, acc) ->
        ...
        :: else ->
            ...
            retain_old = true;
            rx = rx + 1;
            /* send Fin tagged with old tx */
            A2B_SEND(Fin, tx, 0, true);
            goto WAIT_ACK
        fi

    /* responder receives Fin: bump rx+tx and ACK */
    :: A_RECV(Fin, e, kx, acc) ->
        if
        :: PHA_PENDING -> skip
        ...
        :: (completed && retain_old && e == old_rx) ->
            /* duplicate Fin (initiator retransmit after lost ACK): retransmit ACK */
            A2B_SEND(ACK, tx, 0, true)
        :: else ->
            /* ignore stray/duplicate Fin */
            skip
        fi
...
WAIT_ACK:
    ...
    /* Ignore any stray Fin (not expected in WAIT_ACK for initiator). */
    :: A_RECV(Fin, e, kx, acc) ->
        /* discard */
        skip
    ...
    :: (!PHA_PENDING && fin_retries > 0 && len(to_a) == 0) ->
        A2B_SEND(Fin, tx, 0, true);
        fin_retries--
    :: (!PHA_PENDING && fin_retries == 0 && len(to_a) == 0) ->
        /* No further progress possible as initiator after bounded Fin retries. */
        abort_a = true;
        updating = false;
        break
...
WAIT_RESP:
    ...
    :: B_RECV(KeyUpdate, e, kx, acc) -> unexpected = true; break
    :: B_RECV(UnknownEKU, e, kx, acc) -> unexpected = true; break
    :: B_RECV(EarlyEKU, e, kx, acc) -> unexpected = true; break
    :: B_RECV(BadGroupReq, e, kx, acc) -> illegal_parameter = true; break
    :: B_RECV(BadGroupResp, e, kx, acc) -> illegal_parameter = true; break
    ...
    :: (!PHA_PENDING && updating && initiated && !req_acked && req_retries > 0 && len(to_b) == 0) ->
        B2A_SEND(Req, tx, local_kx, true);
        req_retries--
    :: (!PHA_PENDING && updating && initiated && !req_acked && req_retries == 0 && len(to_b) == 0) ->
        /* No further progress possible as initiator after bounded Req retries. */
    ...
    :: B_RECV(Req, e, kx, acc) ->
        ...
        :: ((e == rx) || (retain_old && e == old_rx)) ->
            ...
            :: else ->
                ...
                :: (kx == peer_kx_active) ->
                    if
                    :: (deferred_resp && !resp_sent) -> B2A_SEND(ACK, tx, 0, true)
                    :: else -> B2A_SEND(Resp, tx, local_kx, true)
                    fi
                :: else -> unexpected = true; break
                fi
            fi
...
WAIT_ACK:
    ...
    :: (!PHA_PENDING && fin_retries > 0 && len(to_b) == 0) ->
        B2A_SEND(Fin, tx, 0, true);
        fin_retries--
    :: (!PHA_PENDING && fin_retries == 0 && len(to_b) == 0) ->
        /* No further progress possible as initiator after bounded Fin retries. */
...
proctype ErrorInjector()
{
    ...
    :: (INJECT_ERRORS) ->
        if
        :: A2B_SEND(KeyUpdate, INIT_TX_A, 0, true)
        :: B2A_SEND(KeyUpdate, INIT_TX_B, 0, true)
        :: A2B_SEND(UnknownEKU, INIT_TX_A, 0, true)
        :: B2A_SEND(UnknownEKU, INIT_TX_B, 0, true)
        :: A2B_SEND(EarlyEKU, INIT_TX_A, 0, true)
        :: B2A_SEND(EarlyEKU, INIT_TX_B, 0, true)
        :: A2B_SEND(BadGroupReq, INIT_TX_A, 0, true)
        :: B2A_SEND(BadGroupReq, INIT_TX_B, 0, true)
        :: A2B_SEND(BadGroupResp, INIT_TX_A, 0, true)
        :: B2A_SEND(BadGroupResp, INIT_TX_B, 0, true)
        fi
    :: else -> skip
    fi
}
